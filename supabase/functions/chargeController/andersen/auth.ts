...
const getUsername = async (email: string): Promise<string> => {
  const usernameResponse = await fetch(graphqlProd + '/get-pending-user', {
    method: 'POST',
...
export const signIn = async (authData: { Username: string; Password: string }): Promise<string> => {
  ...
  const result = await authenticateUser(cognitoUser, cognitoAuthenticationDetails);
