<script lang="ts">
  ...
  let { averagePrice, number30mSlots } = $derived.by(() => {
    ...
    return carChargeTimespans.reduce<{ averagePrice: number; number30mSlots: number }>(
      (acc, timespan) => {
        const no30mSlotsInSpan =
          (timespan.endTime.getTime() - timespan.startTime.getTime()) / msIn30m;
        const newNo30mSlots = acc.number30mSlots + no30mSlotsInSpan;
        return {
          averagePrice:
            (acc.averagePrice * acc.number30mSlots + timespan.averagePrice * no30mSlotsInSpan) /
            newNo30mSlots,
