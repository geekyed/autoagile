export const getPrices = async (tariffCode: string): Promise<Price[]> => {
  ...
  try {
    ...
    return rates.results.map(
      (rate: StandardUnitRate): Price => ({
...
export const getTariffCode = async (accountId: string, apiKey: string): Promise<string> => {
  ...
  const electricityMeterPoints: MeterPoint[] = account.properties.flatMap((property) =>
    property.electricity_meter_points.flatMap((meterPoint) => meterPoint.agreements)
  );
