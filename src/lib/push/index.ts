...
const handleNewPrices = (change: RealtimePostgresChangesPayload<Price>) => {
  ...
  if (!flushScheduled) {
    ...
    setInterval(() => {
      ...
      if (batchedUpdates) {
        console.log('returning batched updates to UI', JSON.stringify(batchedUpdates));
        batchedChanges.updates = [...batchedUpdates];
        batchedUpdates = undefined;
      }
      if (batchedInserts) {
        console.log('returning batched inserts to UI', JSON.stringify(batchedInserts));
        batchedChanges.inserts = [...batchedInserts];
        batchedInserts = undefined;
      }
      if (batchedDeletes) {
        console.log('returning batched deletes to UI', JSON.stringify(batchedDeletes));
...
export const subscribeToPriceChanges = (
  ...
  console.log(`subscribing to price changes channel - prices-changes-${tariff}`);
  myCallback = callback;
  return supabase
