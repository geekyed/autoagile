<script lang="ts">
  ...
  $effect(() => {
    if (userProfile?.group.octopusTariff && !channel) {
      channel = subscribeToPriceChanges(
        userProfile?.group.octopusTariff,
        (pricesChanges: PriceChanges) => {
          try {
...
<div class="flex flex-grow flex-col items-center justify-center gap-5">
  {#if prices.length > 0}
    <CreateCarCharge
      carChargingConfig={carChargeConfig}
      bind:carChargeTimespans={chargeTimespans}
    />
